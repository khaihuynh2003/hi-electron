...
public class OrderDAOImpl implements OrderDAO {
    ...
    public List<OrderBean> getAllOrders() {
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                boolean readyForPickup = resultSet.getBoolean("ready_for_pickup");
    ...
    public void updateOrder(OrderBean order) {
        ...
        try {
            connection = DBUtil.provideConnection();
            String sql = "UPDATE orders SET transaction_id = ?, product_id = ?, quantity = ?, readyForPickup = ?, amount = ?, shipped = ? WHERE order_id = ?";
            statement = connection.prepareStatement(sql);
            statement.setString(1, order.getTransactionId());
            statement.setString(2, order.getProductId());
            statement.setInt(3, order.getQuantity());
            statement.setBoolean(4, order.getPickupStatus());
